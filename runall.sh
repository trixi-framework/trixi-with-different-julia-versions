...
for i in 1 2 3; do
  ...
  echo
  echo
  cd julia-1.7-latest-check-bounds-no && ../ttfx.sh julia-1.7 --check-bounds=no && cd ..
  echo
  echo "^^^ These are the results for trial #$i in julia-1.7-latest-check-bounds-no/ ^^^"
  echo
  echo
  cd julia-1.8-latest-check-bounds-no && ../ttfx.sh julia-1.8 --check-bounds=no && cd ..
  echo
  echo "^^^ These are the results for trial #$i in julia-1.8-latest-check-bounds-no/ ^^^"
done
